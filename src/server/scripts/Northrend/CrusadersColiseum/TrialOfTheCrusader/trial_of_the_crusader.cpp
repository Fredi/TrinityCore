            ...
            void UpdateAI(const uint32 uiDiff)
            {
                ...
                if (m_uiUpdateTimer <= uiDiff)
                {
                    switch (m_instance->GetData(TYPE_EVENT))
                    {
                        ...
                        case 6008:
                        {
                            Creature* mage = me->FindNearestCreature(33643, 100.0f, true);
                            mage->SetOrientation(me->GetOrientation());
                            mage->AI()->DoCastAOE(53142);
                        ...
                        case 6009:
                        {
                            ...
                            {
                                Player* portalCaster = itr->getSource();
                                if (!portalCaster->isGameMaster())
                                {
                                    portalCaster->SummonGameObject(191164, posx, posy, posz, 0, 0, 0, 0, 0, 6000000);
                                    break;
                                }
                            }
                            m_uiUpdateTimer = 1000;
